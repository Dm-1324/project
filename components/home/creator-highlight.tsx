...
import { ShoppingBag, ChevronLeft, ChevronRight, Instagram, Youtube, BookText as TikTok } from "lucide-react";
...
export default function CreatorHighlight() {
  ...
  const scrollToCreator = (index: number) => {
    ...
    if (scrollContainerRef.current) {
      const cards = scrollContainerRef.current.querySelectorAll('.creator-card');
      if (cards[index]) {
  ...
  const prevCreator = () => {
    const prevIndex = (activeIndex - 1 + featuredCreators.length) % featuredCreators.length;
  ...
  return (
    <div className="relative">
      ...
      <div 
        ...
        className="flex overflow-x-scroll pb-8 scrollbar-hide snap-x snap-mandatory"
